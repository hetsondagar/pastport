...
def _safe_recovery_response(ranked: List[Tuple[MemoryForRetrieval, float]], user_msg: str) -> str:
    if ranked:
        ...
        return (
            f"I hear your question about '{user_msg}'. "
            f"A key memory to consider is from {top_memory.createdAt.date().isoformat()}: {_excerpt(top_memory.text, 180)}"
        )
    return "I hear your question and suggest focusing on one concrete next step this week."
